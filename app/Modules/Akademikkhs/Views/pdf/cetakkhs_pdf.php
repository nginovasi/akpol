...
<body>
			...
			<table width="100%" border="1" class="tb-aspek">
				...
				<tbody>
					...
					?>
						...
						foreach (json_decode($data[0]['pengetahuan'], true) as $item) {
							$no++; ?>
							<tr>
			...
			<table width="100%" border="1" class="tb-aspek">
				...
				<tbody>
					...
					?>
						...
						foreach (json_decode($data[0]['keterampilan'], true) as $item) {
							$no++; ?>
							<tr>
								<td height="20px"><?= $no ?></td>
								<td align="left" style=""><?= $item['matkul'] ?></td>
			...
			<table width="100%" border="1" class="tb-aspek">
				<thead>
					<tr>
						...
						<th align="right"><?php if (isset($data[0]['rata_keterampilan'])) {
												echo round(($data[0]['rata_pengetahuan'] + $data[0]['rata_keterampilan']) / 2, 2) == 0 ? '' : round(($data[0]['rata_pengetahuan'] + $data[0]['rata_keterampilan']) / 2, 2);
											} ?></th>
					</tr>
				</thead>
			</table>
			...
			<table width="100%" border="1" class="tb-aspek">
				<thead>
					<tr>
						...
						<th align="right"><?php if (isset($data[0]['rata_keterampilan'])) {
												echo round(($rata_pengetahuan + $rata_keterampilan + $rata_karakter + $rata_kesehatan + $rata_jasmani) / 5, 2) == 0 ? '0' : round(($rata_pengetahuan + $rata_keterampilan + $rata_karakter + $rata_kesehatan + $rata_jasmani) / 5, 2);
											} ?></th>
					</tr>
				</thead>
			</table>
	...
	<div style="width: 45%; margin-left: auto;  margin-right: 0;">
		<?php if ($datattd == 1) {
			if ($_GET['trn'] == 1) {
				if (!$datattd->id_taruna == 1) { ?>
					<table width="100%" style="font-size: 12px">
						...
						<tr>
							<td align="center">
								<?php if ($datattd->url_ttd == null || $datattd->url_ttd == '') { ?>
							<td height="50px"></td>
						<?php } else { ?>
							<img src="<?= $datattd->url_ttd ?>">
						<?php } ?>
						</td>
						</tr>
						<tr>
