                                ...
                                <table id="datatable" class="table table-theme table-row v-middle">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="column-2action">Tugas</th>
                                            <th class="column-2action">Proses</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    </tbody>
                    ...
                    <div class="page-content page-container" id="page-content">
                        ...
                        <div class="padding">
                            ...
                            <div class="text-right mt-4">
                                <button class="btn btn-primary" id="idbuttonnilaitugas" style="display: none;" onclick="kirimnilai()">Posting Nilai Tugas</button>
                                <button class="btn btn-primary" id="idbuttonnilaiproses" style="display: none;" onclick="kirimproses()">Posting Nilai Proses</button>
                            </div>
                        </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.showproses', function(event) {
            $('#idbuttonnilaitugas').hide();
            $('#idbuttonnilaiproses').hide();
            $('#collapseKelas').hide();
            event.stopPropagation();
            ...
            if ($this.data('detail') !== undefined) {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        $('#collapseKelas').show();
                        $('#idbuttonnilaiproses').show();
                        if (result.success) {
                            if (result.data.length > 0) {

                                loadTableProses(result, is_ketua_tim);
                                // loadTabelKelas(result, is_ketua_tim);
                                ...
                                Swal.fire('Data Tidak Ditemukan', 'Data nilai proses belum tersedia untuk semester ini', 'warning');
                                $('#table-card').hide();
                                $('#collapseKelas').hide();
                            }
                        } else {
        ...
        let columns = [{
            ...
            },
            {
                ...
                render: function(a, type, data, index) {
                    ...
                    button += '<button class="btn btn-sm btn-outline-primary showproses" type="button" data-id="' + data.id + '" data-detail="' + JSON.stringify(data).replaceAll('"', "'") + '" title="Tampilkan Proses">\
                                    <i class="fa fa-eye"></i>\
                                </button>\
                                ';
    ...
    function loadTableProses(result, is_ketua_tim) {
        if (hot != undefined) {
        ...
        tempdataheader.push("Nilai Awal");
        tempdatacontent.push({
            data: 'na_proses',
            readOnly: true
        });

        tempdataheader.splice(8);
        tempdatacontent.splice(8);
        ...
        for (key in result.data[0]) {
            if (key.includes('pertemuan_ke')) {
                ...
                tempdataheader.push("Pertemuan ke " + tugas_ke);
        ...
        Swal.fire({
            ...
            if (result.value) {
                ...
                $.ajax({
                    ...
                    url: url_insert + '_savenilaiproses',
                    method: "POST",
                    data: {
                        "data": dataarr,
                        "<?= csrf_token() ?>": "<?= csrf_hash() ?>"
                    ...
                    success: function(result) {
                        Swal.close();
                        if (result.success) {
                            Swal.fire('Sukses', 'Berhasil Memposting Proses', 'success');
                            table.ajax.reload();
                        } else {
                            Swal.fire('Error', result.message, 'error');
                        }
