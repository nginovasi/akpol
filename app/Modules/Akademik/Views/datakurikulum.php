                        ...
                        <div class="p-4">
                            ...
                            <div class="d-sm-flex no-shrink mb-4">
                                <div class="px-sm-4 my-3 my-sm-0 flex">
                                    ...
                                    <small class="d-block text-fade" id="nama_semester_modal">Senior Industrial
                                        Designer</small>
                                    <small class="d-block text-fade" id="nama_user_pendidik_modal">Senior Industrial
                                        Designer</small>
                                </div>
                            </div>
                            <div class="row mb-2">
...
<script type="text/javascript">
        ...
        coreEvents.editHandler = {
            ...
            afterAction: function(result) {
                ...
                result.list.forEach(function(list) {
                    ...
                    $row.append($("<td>").html('<input type="text" class="form-control pertemuan_ke" name="pertemuan_ke[' + count + ']" value="' + list.pertemuan_ke + '" oninput="getAllPertemuan(' + count + ')" placeholder="Pertemuan"  required>'));
                    ...
                    if (list.id_jenis_ujian == null) {
                        $(".is_ujian[name='is_ujian[" + count + "]']").val('0');
                    } else {
                        $(".is_ujian[name='is_ujian[" + count + "]']").val(list.id_jenis_ujian);
                    }
                });
                $("#tahun_ajaran").val(result.data.tahun_ajaran);
                $("#id_tingkat").val(result.data.id_tingkat);
                $("#form-simpan").show();
                coreEvents.table.ajax.reload();
                coreEvents.table.page('first').draw('page');
            }
                ...
                if (result.value) {
                    $.ajax({
                        ...
                        data: {
                            ...
                            id_mata_pelajaran: $this.attr('data-id_mata_pelajaran'),
                            "<?= csrf_token() ?>": "<?= csrf_hash() ?>"
                        },
                        dataType: 'json',
                        success: function(result) {
                            Swal.close();
                            if (result.success) {
                                Swal.fire('Sukses', 'Berhasil memverifikasi data kurikulum', 'success');
                                // table.ajax.reload();
                                coreEvents.table.ajax.reload();
                                coreEvents.table.page('first').draw('page');
                            } else {
    ...
    function addrows() {
        ...
        $row.append($("<td>").html('<select class="form-control is_ujian" name="is_ujian[' + count + ']" value="0" placeholder="Pertemuan"  required>\
                                                    <option value="0" active selected >Tidak</option>\
                                                    <option value="1">UTS</option>\
                                                    <option value="2">UAS</option>\
                                                </select>'));
        $row.append($("<td>").html('<input type="text" class="form-control pertemuan_ke" name="pertemuan_ke[' + count + ']" value="" oninput="getAllPertemuan(' + count + ')" placeholder="Pertemuan"  required>'));
    ...
    function numberRows($t) {
        ...
        $t.find("tbody tr").each(function(ind, el) {
            ...
            $(el).find("td:eq(3) .is_ujian").attr("name", "is_ujian[" + ind + "]");
            $(el).find("td:eq(4) .pertemuan_ke").attr("name", "pertemuan_ke[" + ind + "]");
        });
    }

    function getAllPertemuan(idnya) {
        var pertemuan = document.getElementsByClassName('pertemuan_ke');
        var datasama = 0;
        for (var i = 0; i < pertemuan.length; i++) {
            var inp = pertemuan[i];
            if (pertemuan[idnya].value != 0) {
                if (inp.value === pertemuan[idnya].value) {
                    datasama++;
                    if (datasama >= 2) {
                        alert('Pertemuan Sudah Ada')
                        $("input[name='pertemuan_ke[" + idnya + "]']").val('');
                        datasama = 0;
                    }
                }
            }
